defmodule Kryten.Command.Unknown do
  ...
  def handle(message, slack) do
    send_message("<@#{message.user}> I do not understand that. Right now I support `deploy $PROJECT` and `pull requests/PRs` and `failed builds/fb`. Also I will only deploy a project to `staging`.", message.channel, slack)
  end
